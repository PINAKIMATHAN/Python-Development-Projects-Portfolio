...
width, height = 320, 320  # Adjusted for a smaller clock
c_width, c_height = width - 5, height - 5
d = str(width) + "x" + str(height)
my_w.geometry(d)
my_w.title("Analog Clock")
c1 = tk.Canvas(my_w, width=c_width, height=c_height, bg='CadetBlue1')
c1.grid(row=0, column=0, padx=5, pady=5, columnspan=3)
dial = c1.create_oval(10, 10, 310, 310, width=8, outline='chartreuse2', fill='khaki')  # Adjusted size
x, y = 160, 160  # Adjusted center
x1, y1, x2, y2 = x, y, x, 20  # Adjusted
center = c1.create_oval(x - 6, y - 6, x + 6, y + 6, fill='#c0c0c0')
r1 = 140  # Adjusted radius
r2 = 105  # Adjusted radius
rs = 105  # Adjusted radius
rm = 90   # Adjusted radius
rh = 80   # Adjusted radius
...
for i in range(0, 60):
    ...
    if i % 5 == 0:
        ...
        c1.create_text(t1, t2, fill='navy', font="Arial 24 bold", text=next(h))  # Adjusted font size
    else:
        ratio = 0.9
    ...
    c1.create_line(x1, y1, x2, y2, width=2)  # Adjusted line width
...
minute = c1.create_line(x, y, x2, y2, width=3, fill='green')
